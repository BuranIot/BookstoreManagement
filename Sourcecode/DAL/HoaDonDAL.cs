        ...
        public int TaoHoaDon(HoaDonDTO hoaDon, List<ChiTietHoaDonDTO> chiTiets)
        {
            ...
            using (var tran = conn.BeginTransaction())
            {
                try
                {
                    ...
                    int maHoaDon;
                    using (var cmd = new SqliteCommand(sqlHD, conn, tran))
                    {
                        cmd.Parameters.AddWithValue("@Ngay", hoaDon.NgayBan.ToString("yyyy-MM-dd HH:mm:ss"));
                    ...
                    foreach (var ct in chiTiets)
                    {
                        ...
                        using (var cmd = new SqliteCommand(sqlCT, conn, tran))
                        {
            ...
            using (var r    = cmd.ExecuteReader())
            {
                while (r.Read())
                {
                    list.Add(new HoaDonDTO
                    {
                        ...
                        NgayBan = DateTime.Parse(r.GetString(1)),
